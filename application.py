...
from flask import Flask, flash, jsonify, redirect, render_template, request, session            #Jinja2 is a full-featured template engine for Python.it includes files like {% extends "layout.html" %}
from flask_session import Session                                                               # session which allows you to store information specific to a user from one request to the next. This is implemented on top of cookies for you and signs the cookies cryptographically.
...
@app.route("/")
@login_required                                                                       #user must be logged in to order to see what is happening inside that route
def index():
    ...
    total = cash
    stocks = []                            # pass a list of lists to the template page
...
def history():
    ...
    rows = db.execute("SELECT * FROM transactions WHERE user_id = :user", user=session["user_id"])          #sql he toh ata hai:)

    transactions = []                                                                                       # pass a list of lists to the template page
    for row in rows:
        stock_info = lookup(row['symbol'])
    ...
    return render_template("history.html", transactions=transactions)                                       #back to index page
...
def quote():
    ...
    if stock := lookup(request.form.get("symbol")):
...
def sell():
    ...
    if request.method == "POST":
        ...
        stocks = {row['symbol']: row['amount'] for row in rows}
